...
use slirc_client::protocol::{BackendAction, GuiEvent};
...
fn test_backend_thread_creation() {
    ...
    let _handle = std::thread::spawn(move || {
        slirc_client::backend::run_backend(action_rx, event_tx);
    });
...
fn test_action_channel_communication() {
    ...
    action_tx.send(BackendAction::Disconnect).unwrap();
    action_tx
        .send(BackendAction::Join("#test".to_string()))
        .unwrap();
    action_tx
        .send(BackendAction::Part {
...
fn test_gui_event_types() {
    ...
    event_tx
        .send(GuiEvent::Disconnected("test".to_string()))
        .unwrap();
    event_tx
        .send(GuiEvent::Error("test error".to_string()))
        .unwrap();
    event_tx
        .send(GuiEvent::JoinedChannel("#test".to_string()))
        .unwrap();
...
fn test_buffer_state_management() {
    ...
    use slirc_client::protocol::UserInfo;
...
fn test_command_parsing() {
    ...
    assert!(matches!(
        action,
        slirc_client::protocol::BackendAction::Join(_)
    ));
}

#[test]
fn test_protocol_action_serialization() {
    use slirc_client::protocol::BackendAction;
