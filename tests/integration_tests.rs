...
fn test_irc_color_codes_parsing() {
    ...
    for text in test_cases {
        ...
        if text.len() < 20 {
            assert!(
                has_formatting || text == "\x03Reset colors",
                "Expected formatting in: {:?}",
                text
            );
        }
    }
...
fn test_mirc_color_palette() {
    use slirc_client::ui::theme::mirc_color;
